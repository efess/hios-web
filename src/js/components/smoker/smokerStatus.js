...
export default class SmokerStatus extends React.Component {
    ...
    loadCurrentState(gran)  {
        ...
        this.serverRequest = ajax.post('/api/smokes/getSmokerStatus', {
    ...
    newProbe(probe) {
        return ajax.post('/api/smokes/newSession', probe)
            .promise()
    ...
    closeSession(session) {
        return ajax.post('/api/smokes/closeSession', session)
            .promise()
            .then(this.loadCurrentState.bind(this), function _fail(err) {
    ...
    updateTarget(probeId, target) {
        return ajax.post('/api/smokes/updateProbeTarget', {
                probeId: probeId,
                deviceId: '31316536-6633-3939-2d64-6362372d3436',
                target: target
            })
